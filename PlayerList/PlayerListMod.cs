...
[assembly: MelonInfo(typeof(PlayerList.PlayerListMod), "PlayerList", "1.2.0", "loukylor", "https://github.com/loukylor/VRC-Mods")]
    ...
    public class PlayerListMod : MelonMod
    {
        ...
        public static int SnapToGridSize
        {
            ...
            set
            {
                ...
                snapToGridSizeLabel.textComponent.text = $"Snap Grid Size: {value}";
        ...
        public override void VRChat_OnUiManagerInit()
        {
            ...
            new SingleButton(playerListMenus[0].path, new Vector3(-1, 0), "Reset Font", new Action(() => FontSize = 35), "Set font size to the default value (35)", "DefaultFontSizeButton");
            fontSizeLabel = new Label(playerListMenus[0].path, new Vector3(0, 0), $"Font Size: {FontSize}", "FontSizeLabel");
            ...
            foreach (var child in GameObject.Find("UserInterface/QuickMenu/QuickModeMenus").transform)
            {
                EnableDisableListener tabListener = child.Cast<Transform>().gameObject.AddComponent<EnableDisableListener>();
                tabListener.OnEnableEvent += new Action(() =>
                {
            ...
            new SingleButton(playerListMenus[1].path, new Vector3(3, 0), "Edit PlayerList Position", new Action(MovePlayerList), "Click to edit the position of the PlayerList", "EditPlayerListPosButton").text.resizeTextForBestFit = true;

            new SingleButton(playerListMenus[1].path, new Vector3(3, 1), "Move to Right of Quick", new Action(MovePlayerListToEndOfMenu), "Move PlayerList to right side of menu, this can also serve as a reset position button", "LockPlayerListToRightButton").text.resizeTextForBestFit = true;
            ...
            new SingleButton(playerListMenus[1].path, new Vector3(1, 0), "Snap Grid Size +", new Action(() => SnapToGridSize += 10), "Increase the size of the snap to grid by 10", "IncreaseSnapGridSize").text.resizeTextForBestFit = true;
            new SingleButton(playerListMenus[1].path, new Vector3(1, 2), "Snap Grid Size -", new Action(() => SnapToGridSize -= 10), "Decrease the size of the snap to grid by 10", "DecreaseSnapGridSize").text.resizeTextForBestFit = true;
            snapToGridSizeLabel = new Label(playerListMenus[1].path, new Vector3(1, 1), $"Snap Grid Size: {SnapToGridSize}", "SnapToGridSizeLabel");
            snapToGridSizeLabel.textComponent.resizeTextForBestFit = true;
            ...
            for (int i = 0; i < playerListMenus.Count; i++)
            {
                ...
                if (i > 0)
                    new SingleButton(playerListMenus[i].path, new Vector3(4, 0), $"Page {i}", new Action(() => UIManager.OpenPage($"UserInterface/QuickMenu/PlayerListMenuPage{k}")), $"Go back to page {i}", "BackPageButton");
                if (i + 1 < playerListMenus.Count)
                    new SingleButton(playerListMenus[i].path, new Vector3(4, 2), $"Page {i + 2}", new Action(() => UIManager.OpenPage($"UserInterface/QuickMenu/PlayerListMenuPage{k + 2}")), $"Go to page {i + 2}", "ForwardPageButton");
